...
아래 항목은 **이미 끝나 있습니다.** 다시 하실 필요 없습니다.

| | 항목 | 상태 |
|---|---|---|
| 1 | Supabase 무료 조직 `mdmiraecell` | ✅ 완료 |
| 2 | 프로젝트 `mdmiraecell` (서울 리전) | ✅ 완료 · `ghfoffpgkftudwfpldua` |
| 3 | 표 4개 + 접근 권한 + RLS | ✅ 적용됨 |
| 4 | 접수 함수 `submit-consultation` | ✅ 배포됨 (ACTIVE) |
| 5 | `js/config.js` 주소·공개키 | ✅ 채워짐 |
...
| ⓐ | 관리자 계정 만들기 | **필수** | 3번 |
| ⓑ | 회원가입 차단 | **필수** | 3-3 |
| ⓒ | 함수 환경변수 `IP_SALT` 등록 | 권장 | 4번 |
| ⓓ | keepalive 시크릿 등록 | **필수** | 8번 |
| ⓔ | 캡차 · 이메일 알림 | 선택 | 6·7번 |
